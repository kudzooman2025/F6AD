...
function gtRenderNav() {
  ...
  nav.innerHTML = items.map(function(it) {
    var active = (it[0] === page) || (it[0] === 'home' && ['live', 'review', 'new'].indexOf(page) >= 0);
...
function gtGameItem(g) {
  ...
  return '<div class="gt-gitem" onclick="gtGo(\'' + target.slice(1) + '\')">' +
    ...
    '<span class="gi-meta">' + gtFmtDate(g.played_at || g.created_at) + ' · ' + gtEsc(g.game_type || '') + (g.venue ? ' · ' + gtEsc(g.venue) : '') + '</span>' +
    '</div>';
}
